"""Run the preregistered v4r4 He II continuum offline validation."""
...
TEMPERATURE_FLOOR_K = V4R4_FORMAL_TEMPERATURE_FLOOR_K
V4R2_OFFLINE = Path(
...
def _batch_prediction(
    ...
    *,
    ...
    component_sensitivity = {
        ...
        for name in COMPONENT_NAMES_V4R4
    }
    ...
    for start in range(0, labels.shape[0], int(batch_size)):
        ...
        _, window_fraction, sensitivity = _rosseland_diagnostics(
            components,
            component_names=COMPONENT_NAMES_V4R4,
        )
        for index, name in enumerate(COMPONENT_NAMES_V4R4):
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--v4r2-offline", type=Path, default=V4R2_OFFLINE)
    ...
    parser.add_argument(
        ...
        default=Path(
            ...
            "textbook_opacity_v4r4_offline_validation_20260828.json"
        ),
    )
    ...
    for required in (args.v4r3_offline, args.v4r2_offline, args.molecule_ablation):
        if not required.is_file():
            raise SystemExit(f"required prior artifact is missing: {required}")
    ...
    teff_layer_summary = _teff_layer_temperature_summary(
        opacity_residual,
        labels,
        temperature,
        applicable_layers,
    )
    middle_at_least_15000k = next(
        (
            row
            for row in teff_layer_summary
            if row["teff_band"] == "middle_6000_10000K"
            and row["layer_temperature_band"] == "at_least_15000K"
        ),
        None,
    )
    result = {
        "schema_version": 1,
        "candidate": (
            "particle_count_density_h2plus_heminus_heii_hydrogenic_v4r4_domain_4000K"
        ),
        "version": "v4r4",
        "status": status,
        "decision": decision,
        ...
        "historical_v4r2_result_preserved": str(args.v4r2_offline),
        "historical_v4r2_result_sha256": file_sha256(args.v4r2_offline),
        ...
        "v4r4_change": {
            ...
            "helium_minus_density_still_total_helium": True,
            "molecular": False,
        ...
        "teff_x_layer_temperature_summary": teff_layer_summary,
        "middle_band_at_least_15000K": middle_at_least_15000k,
