"""Run the preregistered v4r3 mass-bridge offline validation."""
...
TEMPERATURE_FLOOR_K = V4R3_FORMAL_TEMPERATURE_FLOOR_K
V4R1_OFFLINE = Path(
...
def _batch_prediction(
    ...
    *,
    ...
    component_sensitivity = {
        ...
        for name in COMPONENT_NAMES_V4R3
    }
    ...
    for start in range(0, labels.shape[0], int(batch_size)):
        ...
        _, window_fraction, sensitivity = _rosseland_diagnostics(
            components,
            component_names=COMPONENT_NAMES_V4R3,
        )
        for index, name in enumerate(COMPONENT_NAMES_V4R3):
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--v4r1-offline", type=Path, default=V4R1_OFFLINE)
    ...
    for required in (args.v4r1_offline, args.v4r2_offline, args.molecule_ablation):
    ...
    result = {
        "schema_version": 1,
        "candidate": (
            "particle_count_density_h2plus_heminus_v4r3_domain_4000K"
        ),
        "version": "v4r3",
        "status": status,
        "decision": decision,
        ...
        "historical_v4r1_result_preserved": str(args.v4r1_offline),
        "historical_v4r1_result_sha256": file_sha256(args.v4r1_offline),
        ...
        "v4r3_change": {
            "density_closure": "ideal_gas_particle_count",
