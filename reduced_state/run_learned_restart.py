...
def _reconstruct_worker(payload):
    column_mass, temperature, labels = payload
...
def reconstruct_predicted(
    ...
    *,
    ...
    payloads = [
        (column_mass[i], temperature[i], _label_subset(labels[i]))
    ...
    if not args.production_only:
        ...
        with np.load(prediction_path, allow_pickle=False) as prediction:
            if not np.array_equal(
                prediction["star_indices"][: len(held_out)], held_out
            ):
                raise SystemExit(
                    "prediction artifact was built for a different star set"
                )
            if args.count is None and not np.array_equal(
                prediction["star_indices"], held_out
            ):
                raise SystemExit(
                    "prediction artifact was built for a different star set"
                )
            predicted_column_mass = prediction["column_mass"][: len(held_out)]
            predicted_temperature = prediction["temperature"][: len(held_out)]
        ...
        learned_atmospheres = reconstruct_predicted(
            predicted_column_mass, predicted_temperature, labels, workers=args.workers
        )
        learned_items = [
            (StellarLabels(**_label_subset(labels[i])), learned_atmospheres[i])
