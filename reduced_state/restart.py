...
def run_restart_trial(
    ...
    *,
    ...
    if trial_converged and not _atmosphere_is_finite(result.atmosphere):
        # A finite temperature delta over the structural stop's layer window
        # can be satisfied even when another state field elsewhere in the
        # atmosphere has gone non-finite. This direct, full-state check does
        # not depend on a structured product having been requested, unlike
        # the file-existence proxy below.
